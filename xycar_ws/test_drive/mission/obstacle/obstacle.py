...
right_ultra = 0
obstacle_flag = [0, 0]
none_obs_delay = 4     # 장애물이 탐지되지 않을경우 obstacle mode를 종료할 시간(단위: 초)
angle = 0
# =============================================
...
# =============================================
uldevi_delay = 0.2     # 오른쪽 왼쪽 초음파센서 위치 편차를 보상하는 딜레이(단위: 초)
recover_time = 0.7     # 장애물이 측면에서 사라질때 각도를 회복하는 시간(단위: 초)
recover_angle = 30     # 장애물이 측면에서 사라질때 회복할 각도(단위: 도)
# =============================================
# lidarDriving에서 사용할 변수, 저장공간 선언부
# =============================================
Kp = 0.30
lasterror = 0
right_sepdist =250     # 장애물과 이격할 거리(오른쪽, 단위: mm)
...
start_angle = 140          # 라이다 ROI시작 각도(단위: 도)
end_angle = 220            # 라이다 ROI종료 각도(단위: 도)
...
def recoveryAngle(direction):
    global uldevi_delay, recover_time, angle, recover_angle, speed

    if direction == 0:
        max_time_end = time.time() + uldevi_delay
        while True:
            if time.time() > max_timw_end:
                break
...
def start():
    ...
    global left_ultra_status, right_ultra_status, left_steer_flag, right_steer_flag, speed, angle
    # =========================================
    # ROS 노드를 생성하고 초기화 함.
