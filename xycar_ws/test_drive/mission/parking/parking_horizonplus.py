...
while not rospy.is_shutdown(): # ctrl+C 누르기이전까지 계속 반복
    ...
    if (0.5<=distance2<= 0.55 and parking_flag1==0): # pitch2: 3.945, x: 0.335, y: 0.059, z: 0.443, dis: 0.555, id:2  #artag1을 차량이 1/3정도 지나쳤을 때 artag2
        for _ in range(0,70): # 기존에 약간 오른쪽으로 틀어져서 직진하는 걸 보정하고 더 많이 감
            motor_msg.angle = -3
            motor_msg.speed = 3
            motor_pub.publish(motor_msg)
            if(ultra_msg[4] <= 50 or ultra_msg[5] <= 50): # 초음파센서를 이용해서 수평주차 옆벽 지형을 감지 >> 값은 아직 몰라서 바꿔야 함.
                motor_msg.angle = 0
                motor_msg.speed = 0
                motor_pub.publish(motor_msg)
            rate.sleep()

        for _ in range(0,40): # 기존보다 좀 더 후진
            motor_msg.angle = 50
            motor_msg.speed = -3
            motor_pub.publish(motor_msg)
    ...
    if ( arData[2]["DX"] >0.021 or arData[2]["DX"] < 0.021) and parking_flag3==0 and parking_flag1==1: # 기존에 들어가서 조향보다 좀 더 전후진이 보이도록 하기위해 for문 수정
        if(arData[2]["DX"] >0.021): # 차 기준 중앙보다 왼쪽을 바라 봄
            for i in range(30, -1, -1):
                for _ in range(0,10):
                    motor_msg.angle = 0.6*i
                    motor_msg.speed = 3
                    motor_pub.publish(motor_msg) 
                    rate.sleep()
                for _ in range(0,10):
                ...
                if (arData[2]["DX"] <=0.021): 
                    ...
                    motor_msg.speed = 0
                    break
        elif(arData[2]["DX"] < 0.021): # 차 기준 중앙보다 오른쪽을 바라 봄
            for i in range(-30, 1, 1):
                for _ in range(0,10):
                    motor_msg.angle = 0.6*i
                    motor_msg.speed = 3
