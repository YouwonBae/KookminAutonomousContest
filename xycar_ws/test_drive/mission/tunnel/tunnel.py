...
sub_mission = rospy.Subscriber('H_parking', UInt8, Mode, queue_size=1)
...
Kd = 0.65
lasterror = 0
entry_dist = 500           # 진입으로 판단할 벽과의 거리(오른쪽, 단위: mm)
# =============================================
...
def lidarDriving(right_wall_data, left_wall_data):
    ...
    leftest_wall = np.argmax( left_wall_data[:, 1] )
    leftwall_pos = left_wall_data[leftest_wall, 2]
    leftwall_dist = left_wall_data[leftest_wall, 0]

    rightest_wall = np.argmin( right_wall_data[:, 1] )
    rightwall_pos = right_wall_data[rightest_wall, 2]
    rightwall_dist = right_wall_data[rightest_wall, 0]

    if leftwall_dist < entry_dist and rightwall_dist < entry_dist: # 터널주행을 시작할 벽과의 거리
        tunnel_flag = True 
    else:
        tunnel_flag = False
...
def entryTunnel(raw_distance):
    global distance, tunnel_flag, tunnel_mission_end
    ...
    while(tunnel_flag is True):
        ...
        while start_flag is True:
            ...
            if(angle==50 or angle== -50):
                tunnel_in_flag1 = angle

            if(((tunnel_in_flag1 == 50) and not(angle==50)) or ((tunnel_in_flag1 == -50) and not(angle==-50))):
                tunnel_in_flag2=1
...
def start():
    ...
    global motor, image, distance, wall_flag, tunnel_flag
    # =========================================
    # ROS 노드를 생성하고 초기화 함.
