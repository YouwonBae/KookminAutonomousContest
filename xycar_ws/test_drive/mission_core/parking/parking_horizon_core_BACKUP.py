...
def start():
    ...
    global image, img_ready, motor_msg, arData, id_list, fin, distance1, distance2, cm
    ...
    while not fin: # ctrl+C 누르기이전까지 계속 반복
        ...
        if (0.5<=distance2<= 0.55 and parking_flag1==0): # pitch2: 3.945, x: 0.335, y: 0.059, z: 0.443, dis: 0.555, id:2  #artag1을 차량이 1/3정도 지나쳤을 때 artag2
            ...
            for _ in range(0,36): #35
                ...
                motor_msg.speed = -3
                motor_pub.publish(motor_msg) 
                if(ultra_msg[5] <= 18 or ultra_msg[6] <= 18 or ultra_msg[7] <= 18):
                    motor_msg.angle = 0
                    motor_msg.speed = 0
                    motor_pub.publish(motor_msg)
        ...
        if (pitch2>=40 or pitch2 <= 25) and parking_flag3==0 and parking_flag1==1:
            if(pitch2>=40): # 차 기준 중앙보다 왼쪽을 바라 봄
                for i in range(30, -1, -1):
                    motor_msg.angle = 0.3*i
                    motor_msg.speed = 3
                    motor_pub.publish(motor_msg) 
                    motor_msg.angle = 0.3*i
                    motor_msg.speed = -3
                    ...
                    if (2<pitch2<4.5): 
                        ...
                        motor_msg.speed = 0
                        break
            elif(pitch2<=25): # 차 기준 중앙보다 오른쪽을 바라 봄
                for i in range(-30, 1, 1):
                    motor_msg.angle = 0.3*i
                    motor_msg.speed = 3
                    motor_pub.publish(motor_msg) 
        ...
        if(distance2>=0.21 and parking_flag2==1):
            motor_msg.angle = 0
            motor_msg.speed = 3
            motor_pub.publish(motor_msg) 
            if(distance2<0.21):
