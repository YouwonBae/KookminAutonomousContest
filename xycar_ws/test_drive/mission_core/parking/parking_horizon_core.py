...
from std_msgs.msg import Int32MultiArray, UInt8 # std_msgs.msg 는 퍼블리시를 위해 Int32MultiArray 메시지 유형을 재사용할 수 있도록 하기 위한 것
...
rospy.Subscriber('H_parking', UInt8, Mode, queue_size=1)
...
def start():
    ...
    global image, img_ready, motor_msg, arData, id_list, fin, distance1, distance2
    global roll, pitch, yaw
    global roll1, pitch1, yaw1
    global roll2, pitch2, yaw2

    global parking_flag1, parking_flag2, parking_flag3 
    #=========================================
    # ROS 노드를 생성하고 초기화 함.
    ...
    #=========================================
    motor_msg = xycar_motor()
    while not fin: # ctrl+C 누르기이전까지 계속 반복
        ...
        if (0.5<=distance2<= 0.55 and parking_flag1==0): # pitch2: 3.945, x: 0.335, y: 0.059, z: 0.443, dis: 0.555, id:2  #artag1을 차량이 1/3정도 지나쳤을 때 artag2
            ...
            for _ in range(0,60): # 직진을 좀 더 (+1~3)정도로 조정하고
                motor_msg.angle = -3
                motor_msg.speed = 3
                motor_pub.publish(motor_msg)
                rate.sleep()

            for _ in range(0,30): # 오른쪽으로 핸들 꺾어 후진을 (+-1~3)정도 조정하면 조향 코드빼고 하드코딩만으로도 수평주차 완벽히 가능할지도
                motor_msg.angle = 32
                motor_msg.speed = -4
                motor_pub.publish(motor_msg)
                rate.sleep() # 바퀴를 오른쪽으로 꺾어서 후진하며 주차박스 외곽의 왼쪽의 가운데보다 살짝위에 오른뒷바퀴가 닿으면

            for _ in range(0,30): #35
                motor_msg.angle = -50
                motor_msg.speed = -4
                #motor_pub.publish(motor_msg)
                try:
                ...
                if(ultra_msg[6] <= 25 or ultra_msg[7] <= 25):
                    motor_msg.angle = 0
                    motor_msg.speed = 0
                    motor_pub.publish(motor_msg)
                    break
    ...
    #                     motor_pub.publish(motor_msg) 
    #                     rate.sleep()
    #                     if( ultra_msg[6] <= 19 or ultra_msg[7] <= 19):
    #                         motor_msg.angle = 0
    #                         motor_msg.speed = 0
    #                         motor_pub.publish(motor_msg)
    #                 if (arData[2]["DX"] <=0.030): 
    #                     parking_flag3=1 
    #                     motor_msg.angle = 0
    #                     motor_msg.speed = 0
    #                     break
