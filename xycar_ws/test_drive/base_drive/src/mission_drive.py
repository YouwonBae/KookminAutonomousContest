...
def cbFollowing(data):
    global FollowingMode
    global mission_flag
    if data.data == 0:
        FollowingMode = 0
    elif data.data == 1:
...
rospy.Subscriber('lane_following', UInt8, cbFollowing, queue_size=1)
pub_mission = rospy.Publisher('H_parking', UInt8, queue_size=1)
...
class Driving():
    def __init__(self):
        ...
        self.left_light_white_l = 160 # 진짜160
        self.right_light_white_l = 160 # 160
    ...
    def cbFindLane(self, image, left_or_right):
        ...
        if self.Is_curve in (LEFT, RIGHT):
            ...
            if self.left_fraction <= self.FRACTION_NUM and self.right_fraction <= self.FRACTION_NUM:
                if left_or_right == 'left':
                    self.left_light_white_l -= 2
                    if self.left_light_white_l <= self.min_v:
                        self.left_light_white_l = self.min_v
    ...
    def make_lane(self, image):
        ...
        Kp = 0.35 # 비례 이득 값 # 5 : 0.4,  6 : 0.45        11 : 0.2
        Kd = 0.3   # 미분 이득 값 # 5 : 0.65 6 : 0.65     11 : 0.65
